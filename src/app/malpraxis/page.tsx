    ...
    try {
      ...
      try {
        const consentStatus = await api.get<{ signedDocuments: boolean }>(
          `/online/client/documents/status?legalType=${insured.legalType}&cif=${insured.cif}&vendorProductType=MALPRAXIS`
        );
        if (!consentStatus.signedDocuments) {
          const consentData = await api.get<{
            ...
            `/online/client/documents/fetch-questions?legalType=${insured.legalType}&vendorProductType=MALPRAXIS`
          );
