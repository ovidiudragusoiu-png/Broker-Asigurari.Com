...
export default function PadPage() {
  ...
  const handleCreateOrderAndOffer = async () => {
    ...
    try {
      ...
      const ctName =
        constructionTypes.find((ct) => ct.id === Number(constructionTypeId))?.name || "";

      // 0. Auto-submit consent (same approach as RCA/Travel)
      try {
        const consentStatus = await api.get<{ signedDocuments: boolean }>(
          `/online/client/documents/status?legalType=${contractor.legalType}&cif=${contractor.cif}&vendorProductType=PAD`
        );
        if (!consentStatus.signedDocuments) {
          const consentData = await api.get<{
            ...
            `/online/client/documents/fetch-questions?legalType=${contractor.legalType}&vendorProductType=PAD`
          );
      ...
      // 1. Create order — POST /online/offers/order (non-v3)
      ...
      if (!currentOrderId) {
        const order = await api.post<{ id: number; productType: string; hash: string }>(
          ...
          {
            vendorProductType: "PAD",
            mainInsuredDetails: { ...insuredPerson, policyPartyType: "INSURED", quota: 100 },
            contractorDetails: { ...contractor, policyPartyType: "CONTRACTOR" },
            goodDetails: {
              goodType: "HOME",
              padPropertyType,
              environmentType,
              ...
              addressRequest: propertyAddress,
            },
          }
        );
      ...
      // 2. Create offer — POST /online/offers/paid/pad (non-v3)
      const offerRes = await api.post<PadOfferApi>("/online/offers/paid/pad", {
        ...
        offerDetails: {
          // Renewal fields inside offerDetails per API docs (PaidPadOfferDetailsRequest)
          ...(isRenewal && previousPolicySeries.trim()
            ? { previousPolicySeries: previousPolicySeries.trim() }
            : {}),
          ...(isRenewal && previousPolicyNumber.trim()
            ? { previousPolicyNumber: previousPolicyNumber.trim() }
            : {}),
          notesCesionari: notesCesionari.trim(),
          cesionari: selectedCesionari.map((c) => ({
            cif: c.cif,
            name: c.name,
