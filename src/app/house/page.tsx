    ...
    try {
      ...
      try {
        const consentStatus = await api.get<{ signedDocuments: boolean }>(
          `/online/client/documents/status?legalType=${contractor.legalType}&cif=${contractor.cif}&vendorProductType=HOUSE`
        );
        if (!consentStatus.signedDocuments) {
          const consentData = await api.get<{
            ...
            `/online/client/documents/fetch-questions?legalType=${contractor.legalType}&vendorProductType=HOUSE`
          );
      ...
      const orderGoodDetails = {
        ...
        insuredSumTypeId: 1,
        seismicRiskTypeId: 1,
        addressRequest: houseAddress,
      };

      // Derive dateOfBirth from CNP (required by API for individual insureds)
