...
export default function MalpraxisAgreementsForm({
  ...
  const handleSubmit = async () => {
    ...
    setSubmitError(null);
    try {
      await submitMalpraxisAgreements(personData, answers);
      onComplete();
    } catch (err) {
      const message =
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      {MALPRAXIS_AGREEMENTS_SECTIONS.map((section) => (
                    ...
                    {item.options.map((option) => (
                      ...
                      >
                        <input
                          ...
                          name={item.id}
                          checked={
                            answers[item.id as keyof MalpraxisAgreementAnswers] === option.value
                          }
                          onChange={() =>
                            setAnswers(
                              (prev) =>
